...
def evaluate_loss(model, generator, device):
    """Row-weighted pooled predictive MSE over ``generator`` (no regularization).

    Sum of squared errors divided by the number of rows, so every observation
    carries equal weight regardless of which batch it landed in. Averaging
    per-batch MSEs instead would over-weight a short final batch by the ratio of
    the full batch size to its actual length. This is also what the parallel
    trainer computes, so both execution paths select hyperparameters on
    identical numbers.
    """
    model.eval()
    squared_error = 0.0
